...
function dx($sjh,$type,$yzm,$name,$kdgs,$ydh){
    if($type == 1){//验证码
        $content =urlencode("【云端商城】验证码为：[" . $yzm . "]，欢迎登陆平台！");
    }elseif($type == 2){//发货
        $content = urlencode("【云端商城】[" . $name . "]您好，您的货物已由[" . $kdgs . "]快递公司发出，单号[" . $ydh . "]，请注意查收。");
    }elseif($type == 3){//新订单
        $content = urlencode("【云端商城】已接收到新的订单，请登陆后台查看。");
    }elseif($type == 4){//新工单
        $content =urlencode ("【云端商城】已接收到新的工单，请登陆后台处理。");
    }elseif($type == 5){//工单处理
        $content =urlencode( "【云端商城】您所提交的工单已被客服处理，请登陆至工单列表查看详情。");
    }
    $content = $content . "&mobile=" . $sjh;
    $querys = "content=" . $content;

    $host = "https://cxkjsms.market.alicloudapi.com";
    $path = "/chuangxinsms/dxjk";
    $method = "POST";
    $appcode = "96c3b4a08f544a548906853730515a24";//阿里云接口AppSecret
    $headers = array();
