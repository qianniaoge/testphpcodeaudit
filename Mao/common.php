...
$dbconfig=array(
	...
	'port' => 3306, //数据库端口
	'user' => 'gao3165636' ,//数据库用户名
	'pwd' => '3z6fe2gd', //数据库密码
	'dbname' => 'gao3165636', //数据库名
);

$tx_app_id = '2047155482';//腾讯验证ID
$tx_app_key = '05HwaZsJ6wvKD_3-2IG4_oA**';//腾讯验证KEY
$AppSecret = 'ace64ba8e4674985953764f6a40765f8';//阿里云接口AppSecret
$houzhui = "demo.mzmkj.cn";//分站后缀
...
//
if(!file_exists(SYSTEM_ROOT.'install.back')){
    $install=$DB->query("UPDATE `mao_data` SET `url` = '{$_SERVER['HTTP_HOST']}',`url_1` = '{$_SERVER['HTTP_HOST']}' WHERE `id` = '1';");
    if($install){
        file_put_contents(SYSTEM_ROOT.'install.back','aa11.kuaiyur.com');
    }
}
