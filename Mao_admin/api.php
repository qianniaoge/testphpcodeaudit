...
if($mod){
    ...
    elseif($mod == "set"){
        ...
        $total_sales = daddslashes($_POST['total_sales']);
        $shop_logo = daddslashes($_POST['shop_logo']);
        $shop_name = daddslashes($_POST['shop_name']);
        $shop_bisect = daddslashes($_POST['shop_bisect']);
        if($title == "" || $qq == "" || $wx == "" || $sj == "" || $gd == ""){
            ...
            if($pass != "" || $pass != null){
                if(mb_strlen($pass,'UTF8') < 6 || mb_strlen($pass,'UTF8') > 15){
                    ...
                    $DB->query("update mao_data set title='{$title}',qq='{$qq}',wx='{$wx}',qq='{$qq}',sj='{$sj}',gd_gg='{$gd}',pass='{$pass}',ym_id='{$ym_id}',total_sales='{$total_sales}, shop_logo='{$shop_logo}',shop_name='{$shop_name}', shop_bisect='{$shop_bisect}' where id='{$mao['id']}'");
                    $result=array("code"=>1,"msg"=>"修改成功！");
                }
            }else{
                $DB->query("update mao_data set title='{$title}',qq='{$qq}', wx='{$wx}', qq='{$qq}',sj='{$sj}',gd_gg='{$gd}',ym_id='{$ym_id}' ,total_sales='{$total_sales}' ,shop_logo='{$shop_logo}', shop_name='{$shop_name}', shop_bisect='{$shop_bisect}' where id='{$mao['id']}'");
    ...
    elseif($mod == "list_commodity"){
        ...
        while($res = $DB->fetch($rs)){
            if($res['type'] == 1){
                $type = '<span style="color: #01AAED;">天猫优选</span>';
            }elseif($res['type'] == 2){
                $type = '<span style="color: #01AAED;">超值捡漏</span>';
            }elseif($res['type'] == 3){
                $type = '<span style="color: #01AAED;">人气销量</span>';
            }else{
    ...
    elseif($mod == "add_evaluate"){
        $M_sp = daddslashes($_POST['M_sp']);
        $user_name = daddslashes($_POST['user_name']);
        $user_img = daddslashes($_POST['user_img']);
        $pj_img = daddslashes($_POST['pj_img']);
        $tags = daddslashes($_POST['tags']);
        $pj_img_cot = daddslashes($_POST['pj_img_cot']);
        $pj_text = daddslashes($_POST['pj_text']);
        if($M_sp == "" || $user_name == "" || $user_img == "" || $pj_img == "" || $tags == "" || $pj_img_cot == "" || $pj_text == ""){
            $result=array("code"=>-1,"msg"=>"参数不完整！");
        }else{
            $DB->query("insert into `mao_evaluate` (`M_id`,`M_sp`,`user_name`,`pj_text`,`tags`,`user_img`,`pj_img`,`pj_img_cot`) values ('{$mao['id']}','{$M_sp}','{$user_name}','{$pj_text}','{$tags}','{$user_img}','{$pj_img}','{$pj_img_cot}')");
            $result=array("code"=>0,"msg"=>"添加成功！");
        }
        exit(json_encode($result));
    }//添加评价
    elseif($mod == "set_evaluate"){
        $id = daddslashes($_POST['id']);
        $user_name = daddslashes($_POST['user_name']);
        $user_img = daddslashes($_POST['user_img']);
        $pj_img = daddslashes($_POST['pj_img']);
        ...
        $pj_img_cot = daddslashes($_POST['pj_img_cot']);
        $pj_text = daddslashes($_POST['pj_text']);
        $cha_1 = $DB->get_row("select * from mao_evaluate where M_id='{$mao['id']}' and id='{$id}' limit 1");
        if($id == "" || $user_name == "" || $user_img == "" || $pj_img == "" || $tags == "" || $pj_img_cot == "" || $pj_text == ""){
            $result=array("code"=>-1,"msg"=>"参数不完整！");
        }elseif(!$cha_1){
            $result=array("code"=>-2,"msg"=>"评价不存在！");
        }else{
            $DB->query("update mao_evaluate set user_name='{$user_name}',user_img='{$user_img}',pj_img='{$pj_img}',tags='{$tags}',pj_img_cot='{$pj_img_cot}',pj_text='{$pj_text}' where id='{$cha_1['id']}'");
            $result=array("code"=>0,"msg"=>"修改成功！");
        }
        exit(json_encode($result));
    }//修改评价
    elseif($mod == "del_evaluate"){
        $id = daddslashes($_POST['id']);
        $cha_1 = $DB->get_row("select * from mao_evaluate where M_id='{$mao['id']}' and id='{$id}' limit 1");
        if(!$cha_1){
            $result=array("code"=>-1,"msg"=>"评价不存在！");
        }else{
            $DB->query("DELETE FROM mao_evaluate WHERE M_id='{$mao['id']}' and id='{$cha_1['id']}'");
            $result=array("code"=>0,"msg"=>"删除成功！");
        }
        exit(json_encode($result));
    }//删除商品
    elseif($mod == "list_evaluate"){
        $sql=" e.M_id='{$mao['id']}'";
        $numrows=$DB->count("SELECT count(*) from mao_evaluate e WHERE {$sql} ");
        $pagesize=isset($_REQUEST['limit'])?$_REQUEST['limit']:10;
        $pages=intval($numrows/$pagesize);
        if($numrows%$pagesize){
            $pages++;
        }
        ...
        $offset=$pagesize*($page - 1);
        $rs=$DB->query("SELECT e.*, s.name as sp_name FROM mao_evaluate e left join mao_shop s on e.M_id = s.M_id and e.M_sp = s.id WHERE {$sql} order by id desc limit $offset,$pagesize");
        $data = array();
        while($res = $DB->fetch($rs)){
            $data[]=array(
                'id'=>$res['id'],
                ...
                'user_img'=>'<img src="'.$res['user_img'].'">',
                'pj_text'=>$res['pj_text'],
                'tags'=>$res['tags'],
                'pj_img'=>'<img src="'.$res['pj_img'].'">',
                'pj_img_cot'=>$res['pj_img_cot'],
                'set'=>'<a lay-href="Mao_evaluate/set.php?id='.$res['id'].'" class="layui-btn layui-btn-primary layui-btn-xs">编辑</a><a lay-event="del" class="layui-btn layui-btn-primary layui-btn-xs">删除</a>'
            );
        }
        $first=1;
        $prev=$page-1;
    ...
    elseif($mod == "list_dindan"){
        ...
        $zt = isset($_POST['zt']) ? daddslashes($_POST['zt']) : $_GET['zt'];
        $sql = "M_id='{$mao['id']}' and ";
        if($mao['id'] == 1){
            $sql = "";
        }
        ...
        while($res = $DB->fetch($rs)){
            if($res['zt'] == 0){
                $zt = '<span style="color: white; background-color: red; padding: 6px; font-weight: bold;">未发货</span>';
                $href = 'lay-href="Mao_Order/set.php?id='.$res['id'].'"';
            }elseif($res['zt'] == 1){
                $zt = '<span style="color: white; background-color: #002aff; padding: 6px; font-weight: bold;">未付款</span>';
                $href = '';
            }elseif($res['zt'] == 2){
                $zt = '<span style="color: white; background-color: rgba(55,255,0,1); padding: 6px; font-weight: bold;">已发货</span>';
                $href = 'lay-href="Mao_Order/set.php?id='.$res['id'].'"';
            }elseif($res['zt'] == 3){
                $zt = '<span style="color: #000000; background-color: #ddff00; padding: 6px; font-weight: bold;">异 &nbsp; 常</span>';
                $href = 'lay-href="Mao_Order/set.php?id='.$res['id'].'"';
            }else{
                $zt = '<span style="color: #ff0000; font-weight: bold;">未知</span>';
                $href = '';
            }
